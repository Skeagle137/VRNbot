    ...
    public static long parseTimeString(String s) {
        ...
        for (int i = 0; i < c.length; i++) {
            if (ALLOWED_VALUES.matcher(Character.toString(c[i])).matches()) {
                if (nums.equals(""))
                    throw new VRNException("&cError in position " + (i + 1) + " of the time, you must provide a number before a time unit.");
                matches += 1;
                ...
                if (!ALLOWED_VALUES.matcher(Character.toString(c[i])).matches() && Character.getNumericValue(c[i]) >= 10)
                    throw new VRNException("&cError in position " + (i + 1) + " of the time, unknown time unit. The accepted values are seconds (s), minutes (m), hours (h), days (d), or years (y).");
                throw new VRNException("&cError in parsing the time, please check if you formatted the time correctly.");
            }
        }
        if (!nums.equals("") && matches == 0 || tempunit.equals("")) {
            if (!ALLOWED_VALUES.matcher(s.substring(c.length - 1)).matches() && Character.getNumericValue(s.charAt(c.length - 1)) >= 10)
                throw new VRNException("&cError in position " + c.length + " of the time, unknown time unit. The accepted values are seconds (s), minutes (m), hours (h), days (d), or years (y).");
            throw new VRNException("&cError in position " + c.length + " of the time, you must specify the time unit.");
        }
        return num;
    }
